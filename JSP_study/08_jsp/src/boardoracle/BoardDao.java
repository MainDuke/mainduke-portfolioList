	...
	public void insertArticle(BoardDto dto) throws Exception{
		...
		try {
			...
			*/
			...
			sql="insert into board3(num, writer,email,subject,passwd,regdate,ref,re_step,re_level,content,ip)"
					+" values(board3_seq.NEXTVAL, ?,?,?,?,sysdate,?,?,?,?,?)";
	...
	public List getList(int start, int cnt) throws Exception{
		...
		try {
			...
			*/
			...
			//oracle은 subQuery 를 이용해서 한다.
			
			String sql="select * from (select rownum rnum,num, writer, email, subject,"
					+ " passwd, regdate, readcount, ref, re_step, re_level, content, ip from(select * from board3 order by ref desc, re_step asc)) "
					+ "where rnum>? and rnum<=?";
